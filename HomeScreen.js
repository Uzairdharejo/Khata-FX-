...
// This screen reads directly from the LOCAL database — works with no internet.
export default function HomeScreen({ navigation }) {
  const [transactions, setTransactions] = useState([]);

  useEffect(() => {
    const txCollection = database.get('transactions');
    // observe() gives live updates — the list refreshes instantly when a new
    // entry is saved, no manual refresh or network call needed.
